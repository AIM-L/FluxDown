...
section: api
order: 2
sourceHash: "b251ebfc6d68"
---

FluxDown 提供一个命令行客户端 `fluxdown`，对标 `aria2c`。它是一个薄的 typed HTTP 客户端，运行在[管理 API](/docs/zh/api/overview/) 之上：每条命令都与运行中的 FluxDown 桌面应用或 [Headless 服务器](/docs/zh/headless-server/setup/) 通信（默认 `http://127.0.0.1:17800`，也可指向任意地址），因此它管理的正是你在应用里看到的同一批任务与队列。

它不是独立的下载器 —— 需要有一个正在运行的 FluxDown 实例可供连接。可以把它理解为一个可脚本化的远程遥控器，扮演的角色与 `aria2c` RPC 客户端模式相同。
...
CLI 只是驱动同一套管理 API 的方式之一。若你已有面向 aria2 的工具链，[aria2 兼容 RPC 端点](/docs/zh/api/overview/#curl-示例)可能更合适；面向 AI 客户端则用 [MCP](/docs/zh/api/overview/#mcpmodel-context-protocol)。它们操作的是同一批任务与队列。Metalink、XML-RPC、以及 aria2 的会话保存/恢复刻意未实现 —— FluxDown 的 SQLite 存储已在重启间持久化了一切。
